    ...
    text_field_embedder+: {
      token_embedders+: {
        tokens+: {
          ...
          // embeddings from Common Crawl with a restricted vocabulary, I guess the uncased one. So the best idea is to
          // use the one that seems the same corpus (maybe the exact same embeddings) but restricting the vocabulary.
