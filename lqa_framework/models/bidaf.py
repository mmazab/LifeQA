...
from allennlp.modules import FeedForward, Highway, Seq2VecEncoder, SimilarityFunction
...
class BidirectionalAttentionFlow(LqaClassifier):
    ...
    * The code sections associated with spans are removed.
    ...
    def __init__(self,
        ...
        self._time_distributed_highway_layer = TimeDistributed(self._highway_layer)
    ...
    def forward(self, question: Dict[str, torch.LongTensor], answers: Dict[str, torch.LongTensor],
        ...
        for key in captions:
            captions[key] = captions[key].squeeze(1)
        ...
        embedded_answers = self._time_distributed_highway_layer(self._text_field_embedder(answers, num_wrapping_dims=1))
        answers_mask = util.get_text_field_mask(answers, num_wrapping_dims=1)
        encoded_answers = self._answers_encoder(embedded_answers, answers_mask)  # FIXME

        fuse_cq = self._classifier_feedforward(modeled_passage)
