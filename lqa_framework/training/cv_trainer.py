...
from allennlp.training import NoOpTrainer, TrainerBase, util as training_util
...
class CrossValidationTrainer(TrainerBase):
    ...
    def __init__(self,
        ...
        if not self.leave_model_trained and self.final_validation_dataset:
            raise ValueError("The final validation dataset can only be set if the model is set be to be left trained.")
    ...
    def _build_subtrainer(self, serialization_dir: str, train_dataset: Iterable[Instance],
        ...
        if subtrainer_type == 'default':
            ...
            raise ValueError(f"Subtrainer type '{self.subtrainer_type}' not supported."
    ...
    def train(self) -> Dict[str, Any]:
        ...
        for fold_index, (train_indices, validation_indices) in enumerate(self.cross_validation_splitter(self.dataset,
                                                                                                        groups=groups)):
        ...
        if self.leave_model_trained:
            subtrainer = self._build_subtrainer(self._serialization_dir, self.dataset, self.final_validation_dataset)
            subtrainer.train()
